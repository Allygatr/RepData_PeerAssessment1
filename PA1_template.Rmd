...
```{r Loading and processing data, message=FALSE, warning=FALSE}
...
```{r Calculating thr total number of steps per day, as well as the mean and median}
...
The mean of the total number of steps per day is `r mean_steps` and 
the median is `r median_steps`. It is worth noting that the mean and median are
...
```{r Calculating the total number of steps, mean and median}
...
```
```{r histogram of imputed data, fig.height= 5}
...
But there has been no change in the mean and median, the median (`r imp_median_steps`) 
and mean (`r imp_mean_steps`), for the imputed data set are the same. 
...
```{r Difference between weekday and weekends, message=FALSE, warning=FALSE}
...
```
```{r Panel plot of time series plots, fig.height= 5}
